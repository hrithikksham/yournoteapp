...
// Response interceptor to handle automatic token refreshes
...
// --- Journal API Endpoints (api/journal.ts) ---
const journalApi = {
  /** Creates a new journal entry. (POST /api/journal/) */
  createJournal: async (data: any) => {
    const token = await SecureStore.getItemAsync('access_token');
    return apiClient.post('/api/journal/', data, {
      headers: { Authorization: `Bearer ${token}` },
    });
  },
  /** Fetches a single journal entry by its ID. (GET /api/journal/{id}) */
  getJournalById: async (id: string) => {
    const token = await SecureStore.getItemAsync('access_token');
    return apiClient.get(`/api/journal/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  },
  /** Updates an existing journal entry. (PUT /api/journal/{id}) */
  updateJournal: async (id: string, data: any) => {
    const token = await SecureStore.getItemAsync('access_token');
    return apiClient.put(`/api/journal/${id}`, data, {
      headers: { Authorization: `Bearer ${token}` },
    });
  },
  /** Uploads an image for a journal entry. (POST /api/upload/image/journal) */
  uploadJournalImage: async (imageUri: string) => {
    ...
    const endpoint = '/api/upload/image/journal'; // Make sure this endpoint exists in your backend
  ...
  /** Returns the base URL for constructing full image paths. */
...
// --- Main AddJournalScreen Component ---
export default function AddJournalScreen() {
  ...
  // --- State updated to match new schema (no title, no isLocked) ---
  ...
  const fetchData = async () => {
    if (!journalId) {
      if (entryDateParam) setEntryDate(new Date(entryDateParam));
      return;
    }
    setIsLoading(true);
    try {
      ...
      // No need to set title or lock state
    } catch (error) {
  ...
  // --- Save logic updated to match JournalEntryCreate schema ---
  const handleSaveJournal = async () => {
    // No title validation needed anymore
    if (!content.trim()) {
    ...
    try {
      ...
      const journalData = { 
        content, 
        // Format date to YYYY-MM-DD as expected by Pydantic 'date' type
  ...
  // --- Menu actions updated (Lock/Unlock removed) ---
  const menuActions = [
  ...
  // --- ParsedContent logic remains the same ---
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.contentEditor}>
              {/* Simplified editor view, always editable */}
